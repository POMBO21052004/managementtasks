...
export default function EmployeeNavbar({ toggleSidebar }) {
  ...
  return (
    <Navbar className="navbar fixed-top" expand={false}>
      ...
      <div className="navbar-right">
        {/* Indicateur de rôle */}
        <Badge bg="outline-primary" className="me-2 d-none d-sm-block">
          Employé
        </Badge>
        ...
        <Dropdown align="end" className="user-dropdown" onToggle={(isOpen) => {
          ...
          >
            ...
            <span className="user-name d-none d-sm-inline">
              {truncateName(user?.first_name || "Employé")}
            </span>
          ...
          <Dropdown.Menu className={`dropdown-menu ${theme === "dark" ? "bg-dark border-secondary" : ""}`}>
            ...
            <div className="px-3 py-2 border-bottom">
              <div className="fw-semibold">{user?.first_name || "Employé"}</div>
              <div className="text-muted small">{user?.email || "employe@woopservice.com"}</div>
              <div className="text-primary small">Employé</div>
            </div>
